...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (fetchBinanceRsiDataBtn) {
        fetchBinanceRsiDataBtn.addEventListener('click', () => fetchData('http://trendex.run/rsis/binance', 'result-binance-rsi'));
    }

    const fetchUpbitRsiDataBtn = document.getElementById('fetchUpbitRsiDataBtn');
    if (fetchUpbitRsiDataBtn) {
        fetchUpbitRsiDataBtn.addEventListener('click', () => fetchData('http://trendex.run/rsis/upbit', 'result-upbit-rsi'));
    }

    const fetchUpbitMacdDataBtn = document.getElementById('fetchUpbitMacdDataBtn');
    if (fetchUpbitMacdDataBtn) {
        fetchUpbitMacdDataBtn.addEventListener('click', () => fetchData('http://trendex.run/macds/upbit', 'result-upbit-macd'));
    }

    const fetchBinanceMacdDataBtn = document.getElementById('fetchBinanceMacdDataBtn');
    if (fetchBinanceMacdDataBtn) {
        fetchBinanceMacdDataBtn.addEventListener('click', () => fetchData('http://trendex.run/macds/binance', 'result-binance-macd'));
    }
});
